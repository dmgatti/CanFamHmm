...
#obs.dir  = "/projects/dgatti/Dog/GeneseekData/Cleaned/"
#emis.dir  = "/data/dgatti/Dog/InputData/PureBreedsAllHQ/"
#output.dir = "/projects/dgatti/Dog/HMM/HMM_v6/win1/"
#window = 1
#dog = 1
...
# Set the transtion probs multiplier.
trans.factor = 1e-15
...
# NEED TO STANDARDIZE INPUT FILE NAMES.
  obs = readRDS(paste0(obs.dir, "jax_chr", chr, ".phased.rds"))
...
# NEED TO STANDARDIZE MAP FILE NAMES.
  ...
  probs[[chr]] = forwardBackward(obs = as.integer(obs[dog,]), emis = emis, 
                 map = map[,3] * trans.factor, window = window)
  ...
  vit[[chr]] = viterbi(as.integer(obs[dog,]), emis = emis,
               map = map[,3] * trans.factor, window = window) + 1
