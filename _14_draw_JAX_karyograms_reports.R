...
# Names of pure bred dogs that need a narrower bar plot.
pure.names = c("Bella", "Ben", "Brady", "Cali", "Charlie", "Emma",
               "Hugo", "Jefferson", "Jellybean", "Koko", "Kreuza",
               "Loki", "Parkman", "Shadow", "Sky", "Storm", "Tali",
               "Thunder")
...
for(d in dog.names) {
  ...
  png(paste0("../reports/", d, "_karyogram.png"), width = 2000, height = 2400, res = 200)
  dog_karyogram(hapA = hap1, hapB = hap2, map = map, breeds = breeds,
                title = d, sex = sex[d], colors = 12)
  dev.off()

  # Make the plots narrower for pure breeds with only one breed in the barplot.
  if(d %in% pure.names) { 
    png(paste0("../reports/", d, "_barplot.png"), width = 800, height = 800, res = 200)
  } else{ 
    png(paste0("../reports/", d, "_barplot.png"), width = 2000, height = 1200, res = 200)
  }
